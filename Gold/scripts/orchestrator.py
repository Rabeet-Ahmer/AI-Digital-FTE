"""
Orchestrator (Gold Tier) - Master process that coordinates the AI Employee.
...
5. Update Dashboard.md with current state (incl. Social Media, Odoo ERP, API Rate Limits)
6. Move completed items to /Done
7. Flag stale approvals (>48h)
8. Maintain audit logs

Gold tier adds: Meta (FB/IG), Twitter/X, Odoo ERP integration.
"""
...
class GoldOrchestrator:
    ...
    def run(self):
        ...
        self.logger.info(f"Gold Orchestrator started. Vault: {self.vault_path}")
    ...
    def _process_item(self, item_path: Path):
        ...
        prompt = (
            f"You are the AI Employee (Gold Tier). "
            ...
            f"4. If an outbound action is needed (email send, LinkedIn post, Meta post, tweet, delete, financial), "
    ...
    def _check_approved_items(self):
        ...
        for item_path in approved_files:
            ...
            prompt = (
                ...
                f"2. Execute the action (send email, post to LinkedIn/Meta/Twitter, Odoo financial, delete, etc.)\n"
...
| Meta (FB/IG) API   | Standby | {now.isoformat()} |
| Twitter/X API      | Standby | {now.isoformat()} |
| Odoo ERP (MCP)     | Standby | {now.isoformat()} |
...
| Weekly (Fri) | Odoo Financial Summary | -- | -- | Configured |
...
def main():
    ...
    orchestrator = GoldOrchestrator(str(vault_path), poll_interval=30)
    orchestrator.run()

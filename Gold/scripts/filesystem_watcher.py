"""
File System Watcher (Gold Tier) - Monitors /Inbox and /Approved folders.

Handles: file triage, LinkedIn posts, Meta (FB/IG) posts, tweets, and financial actions.
...
When an approved action appears in /Approved, dispatches to the appropriate handler:
- linkedin_post → linkedin_poster.py (Playwright)
- meta_post → meta_poster.py (Graph API)
- tweet → twitter_poster.py (API v2)
- financial → signals orchestrator (Odoo MCP)
"""
...
class ApprovedHandler(FileSystemEventHandler):
    ...
    def _dispatch(self, source: Path, content: str, action_type: str):
        ...
        if action_type == "linkedin_post":
            ...
            self._handle_meta_post(source, content)
        elif action_type == "tweet":
            self._handle_tweet(source, content)
        elif action_type == "financial":
            # Financial actions are handled by orchestrator via Odoo MCP
            self.logger.info("Financial action — signaling orchestrator for Odoo MCP execution")
            self._write_signal()
            self._log_action(
                "approved_dispatch",
                str(source),
                "",
                "Approved financial action — signaled orchestrator for Odoo MCP",
                "pending",
            )
        else:
    ...
    def _extract_proposed_content(self, content: str) -> str | None:
        """Extract text from the '### Proposed Content' section of an approval file."""
        in_proposed = False
        lines = []
        for line in content.split("\n"):
            if line.strip() == "### Proposed Content":
    ...
    def _extract_metadata_field(self, content: str, field: str) -> str | None:
        """Extract a field value from frontmatter or proposed_action block."""
        for line in content.split("\n"):
            stripped = line.strip()
    ...
    def _handle_meta_post(self, source: Path, content: str):
        """Extract post content and run meta_poster.py."""
        post_content = self._extract_proposed_content(content)
        if not post_content:
            self.logger.error("Could not extract post content from Meta approval file")
            self._log_action("error", str(source), "", "No post content found", "failure")
            return

        platform = self._extract_metadata_field(content, "platform") or "facebook"
        image_url = self._extract_metadata_field(content, "image_url")

        tmp_file = self.scripts_dir / ".tmp_meta_content.txt"
        tmp_file.write_text(post_content, encoding="utf-8")

        self.logger.info(f"Posting to Meta/{platform} ({len(post_content)} chars)...")

        try:
            import subprocess

            cmd = [
                "uv", "run", "python", "meta_poster.py",
            ...
            if result.returncode == 0:
                self.logger.info(f"Meta/{platform} post result: {result.stdout.strip()[:200]}")
                self._complete_item(source, result.stdout.strip())
            else:
                self.logger.error(f"Meta/{platform} post failed: {result.stderr[:200]}")
                self._log_action("error", str(source), "", result.stderr[:200], "failure")

        except subprocess.TimeoutExpired:
            self.logger.error(f"Meta/{platform} posting timed out")
            self._log_action("error", str(source), "", "Posting timed out", "failure")
        finally:
            tmp_file.unlink(missing_ok=True)

    def _handle_tweet(self, source: Path, content: str):
        """Extract tweet content and run twitter_poster.py."""
        post_content = self._extract_proposed_content(content)
        if not post_content:
            self.logger.error("Could not extract tweet content from approval file")
            self._log_action("error", str(source), "", "No tweet content found", "failure")
            return

        tmp_file = self.scripts_dir / ".tmp_tweet_content.txt"
        tmp_file.write_text(post_content, encoding="utf-8")

        # Detect thread format (tweets separated by |||)
        is_thread = "|||" in post_content

        self.logger.info(f"Posting to Twitter ({'thread' if is_thread else 'single'}, {len(post_content)} chars)...")

        try:
            import subprocess

            if is_thread:
                cmd = ["uv", "run", "python", "twitter_poster.py", "--thread", post_content]
            else:
                cmd = ["uv", "run", "python", "twitter_poster.py", "--content-file", str(tmp_file)]
            ...
            if result.returncode == 0:
                self.logger.info(f"Twitter post result: {result.stdout.strip()[:200]}")
                self._complete_item(source, result.stdout.strip())
            else:
                self.logger.error(f"Twitter post failed: {result.stderr[:200]}")
                self._log_action("error", str(source), "", result.stderr[:200], "failure")

        except subprocess.TimeoutExpired:
            self.logger.error("Twitter posting timed out")
            self._log_action("error", str(source), "", "Posting timed out", "failure")
        finally:
            tmp_file.unlink(missing_ok=True)
